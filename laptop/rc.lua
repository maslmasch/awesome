...
usb_handling = require("usb_handling")
...
mymusbmenu , mynmusbmenu = usb_handling.menu_builder()
...
local testmenu = radical.context{style= radical.style.classic}
testmenu:add_item {text="Mounted Devices", sub_menu = function()
        local smenu1 = radical.context{style= radical.style.classic}
        mymusbmenu , mynmusbmenu = usb_handling.menu_builder()
        for i = 1, #mymusbmenu do 
            temp_cmd = "sudo /.script/usb_mount_script.py umount "..mymusbmenu[i][1].." "..mymusbmenu[i][5]
            smenu1:add_item{text=mymusbmenu[i][1],button3=function()
                  os.execute(temp_cmd)
                  testmenu:hide()
              end,
              button1=function() awful.util.spawn_with_shell("urxvt -cd ".. mymusbmenu[i][5]) testmenu:hide() end}
        end
        return smenu1
end}
testmenu:add_item {text="Not Mounted Devices", sub_menu = function()
        local smenu2 = radical.context{style= radical.style.classic}
        mymusbmenu , mynmusbmenu = usb_handling.menu_builder()
        for i = 1, #mynmusbmenu do 
            temp_cmd = "sudo /.script/usb_mount_script.py mount "..mynmusbmenu[i][1]
            smenu2:add_item{text=mynmusbmenu[i][1],button3=function()
                  os.execute(temp_cmd)
                  testmenu:hide()  
              end}
        end
        ...
        elseif args[2] < 10 and state=="Battery:" then
            if( args[2] < 5 ) then
                naughty.notify({ preset = naughty.config.presets.critical,
                                 ...
                                 text = "Only ".. args[3] .. " minutes are remaing before system failure!" })
...
vicious.register(usedspacewidget, vicious.widgets.fs, 
		...
		return '/: ' .. args["{/ used_gb}"] .. " / "..args["{/ size_gb}"].." GB   "
			.."/home: " .. args["{/home used_gb}"].." / "..args["{/home size_gb}"].. "GB"
	end,769)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey, "Shift"   }, "m",     function () awful.util.spawn(terminal .. " -e bash user_scripts/xrandr_small_menu " ) end),
